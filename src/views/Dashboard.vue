  ...
  methods: {
    ...
    async getClients(){
      try {
          ...
          this.clientsCount = getClients.data.length
          getClients.data.forEach(element => {
            this.pcCount = this.pcCount + element.items.length
          });
      }catch(err){
          if (!err.response) {
              this.$swal({
    ...
    async getTicketsMonth(){
      try {
          const getTickets = await axios.get(endPoint.endpointTarget+'/tickets/monthtickets')
          this.monthTickets = getTickets.data.length
      }catch(err){
          if (!err.response) {
              this.$swal({
                  customClass: {container:"fixModal"},
    ...
    async getTickets(){
      try {
          ...
          this.ticketsCount = getTickets.data.length
      }catch(err){
          if (!err.response) {
              this.$swal({
                  customClass: {container:"fixModal"},
